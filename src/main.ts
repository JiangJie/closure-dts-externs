...
export interface GenerateExternsOptions {
    ...
    /**
     * Extra global variable names to include in the output header.
     *
     * @example
     * ```ts
     * extraGlobalVars: ['myGlobal']
     * ```
     */
    extraGlobalVars?: string[];
...
function buildExternsContent(
    ...
    for (const v of extraGlobalVars) {
        allGlobalNames.add(v);
    }
...
export function generateExterns(options: GenerateExternsOptions): string {
    ...
    const content = buildExternsContent(globalVars, globalFunctions, interfaceMembers, extraGlobalVars);
