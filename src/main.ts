    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        ts.forEachChild(sourceFile, node => {
            if (ts.isModuleDeclaration(node) && node.name && ts.isIdentifier(node.name)) {
                ...
                if (exclude?.(nsName, { kind: 'namespace' })) return;

                visitNamespace(node, interfaceMembers, nsName, exclude);
            } else if (ts.isVariableStatement(node)) {
                for (const decl of node.declarationList.declarations) {
                    const varName = getNodeName(decl);
                    if (!varName || exclude?.(varName, { kind: 'variable' })) continue;
                ...
                if (fnName && !exclude?.(fnName, { kind: 'function' })) {
                    globalFunctions.add(fnName);
                }
            } else if (ts.isInterfaceDeclaration(node)) {
                const name = node.name.text;
                if (exclude?.(name, { kind: 'interface' })) return;

                mergeMembers(interfaceMembers, name, collectMembers(node, '', name, exclude));
            } else if (ts.isClassDeclaration(node) && node.name) {
                const name = node.name.text;
                if (exclude?.(name, { kind: 'class' })) return;
...
function defaultFileFilter(filePath: string): boolean {
    return !filePath.includes('/typescript/lib/');
}
...
function collectMembers(
    ...
    ts.forEachChild(node, child => {
        ...
        if (exclude?.(memberName, { kind: 'member', scope })) return;
        ...
        if ((ts.isPropertySignature(child) || ts.isPropertyDeclaration(child)) && child.type && ts.isTypeLiteralNode(child.type)) {
            for (const nested of collectMembers(child.type, path, `${scope}.${memberName}`, exclude)) {
                members.add(nested);
            }
        }
    });
...
function visitNamespace(
    ...
    ts.forEachChild(body, child => {
        if (ts.isModuleDeclaration(child) && child.name && ts.isIdentifier(child.name)) {
            ...
            if (exclude?.(nestedName, { kind: 'namespace', scope: namespacePath })) return;

            visitNamespace(child, interfaceMembers, `${namespacePath}.${nestedName}`, exclude);
        } else if (ts.isInterfaceDeclaration(child)) {
            const name = child.name.text;
            if (exclude?.(name, { kind: 'interface', scope: namespacePath })) return;

            const memberScope = `${namespacePath}.${name}`;
            mergeMembers(interfaceMembers, name, collectMembers(child, '', memberScope, exclude));
        } else if (ts.isClassDeclaration(child)) {
            const name = child.name?.text;
            if (!name) return;
            if (exclude?.(name, { kind: 'class', scope: namespacePath })) return;
